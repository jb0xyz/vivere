...
Vivere は MIT ライセンスのオープンソースプロジェクトです。リポジトリは [github.com/jb0xyz/vivere](https://github.com/jb0xyz/vivere) にあります。npm パッケージ名は `@jb0xyz/vivere` です。現在のパッケージバージョンは `0.3.0` です。まだ初期段階ですが、ファイルベースのコマンドとコンポーネントの流れは動作します。
