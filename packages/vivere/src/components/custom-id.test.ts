...
describe('customId', () => {
  ...
  test('throws when signed payload is changed', () => {
    const raw = encodeCustomId('button', 'confirm', { userId: '123456789012345678' }, secret)
  ...
  test('throws when payload exceeds 100 chars', () => {
    expect(() => encodeCustomId('button', 'x', { blob: 'a'.repeat(200) }, secret)).toThrow(CustomIdTooLongError)
  })

  test('throws when id contains unsupported characters', () => {
    expect(() => encodeCustomId('button', 'role:confirm', {}, secret)).toThrow('Invalid customId id')
  })

  test('throws for malformed encoded values', () => {
    expect(() => decodeCustomId('c1', secret)).toThrow('Malformed customId')
    expect(() => decodeCustomId('c1:button:::sig', secret)).toThrow('Missing customId id')
  })

  test('throws when component kind is missing', () => {
    expect(() => decodeCustomId('c1::confirm::sig', secret)).toThrow('Missing customId component kind')
  })
})
